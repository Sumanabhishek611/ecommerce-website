...
const RecentHistoryPage = () => {
  ...
  useEffect(() => {
    ...
    const history = [...(user.clickedProducts || [])];
    const seen = new Set();
    const recentClickedProducts = [];
    history.forEach(item => {
      const pid = item.productId?._id ?? item.productId;
      const prod = allProducts.find(p => p._id === pid || String(p._id) === String(pid));
      if (prod && !seen.has(prod._id)) {
        seen.add(prod._id);
        recentClickedProducts.push(prod);
      }
    });
    setRecentProducts(recentClickedProducts.slice(0, 20));
    setLoading(false);
    ...
    <div className="min-h-screen bg-blue-50/50 pt-20">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Recent History</h1>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 bg-white rounded-lg border border-blue-100">
            <p className="text-gray-500 mb-4">No recent history yet</p>
            <button
              onClick={() => navigate('/')}
